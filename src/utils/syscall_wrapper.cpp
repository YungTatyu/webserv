...
int syscall_wrapper::Access(const std::string &path, int modes, bool err_log) {
  ...
  if (ret == -1 && err_log) WebServer::writeErrorlog(error::strSysCallError("access", path), config::WARN);
  return ret;
}
...
int syscall_wrapper::Setsockopt(int socket, int level, int option_name, const void *option_value,
  ...
  if (re == -1) WebServer::writeErrorlog(error::strSysCallError("setsockopt"), config::WARN);
  return re;
}
...
int syscall_wrapper::Accept(int socket, struct sockaddr *address, socklen_t *address_len) {
  ...
  if (connfd == -1) WebServer::writeErrorlog(error::strSysCallError("accept"), config::WARN);
  return connfd;
}
...
int syscall_wrapper::Dup2(int fildes, int fildes2) {
  ...
  if (re == -1) WebServer::writeErrorlog(error::strSysCallError("dup2"), config::WARN);
  return re;
}
...
int syscall_wrapper::Fcntl(int fd, int cmd, int flags) {
  ...
  if (re == -1) WebServer::writeErrorlog(error::strSysCallError("fctnl"), config::WARN);
  return re;
}

int syscall_wrapper::Getsockname(int sockfd, struct sockaddr *addr, socklen_t *addrlen) {
  int re = getsockname(sockfd, addr, addrlen);
  if (re == -1) WebServer::writeErrorlog(error::strSysCallError("getsockname"), config::WARN);
  return re;
}
