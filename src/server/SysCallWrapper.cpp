...
int SysCallWrapper::Socket(int domain, int type, int protocol) {
  ...
  if (listenfd == -1) 
    throw std::runtime_error(error::strSysCallError("socket"));
  return listenfd;
}
...
int SysCallWrapper::Setsockopt(int socket, int level, int option_name, const void *option_value,
  ...
  if (re == -1)
    std::cerr << error::strSysCallError("setsockopt") << std::endl;
  return re;
}
...
int SysCallWrapper::Bind(int socket, const struct sockaddr *address, socklen_t address_len) {
  ...
  if (re == -1)
    throw std::runtime_error(error::strSysCallError("bind"));
  return re;
}
...
int SysCallWrapper::Accept(int socket, struct sockaddr *address, socklen_t *address_len) {
  ...
  if (connfd == -1)
    std::cerr << error::strSysCallError("accept") << std::endl;
  return connfd;
}
...
pid_t SysCallWrapper::Fork(void) {
  int re = fork();
  if (re == -1)
    std::cerr << error::strSysCallError("fork") << std::endl;
  return re;
}
...
int SysCallWrapper::Dup2(int fildes, int fildes2) {
  ...
  if (re == -1)
    std::cerr << error::strSysCallError("dup2") << std::endl;
  return re;
}
