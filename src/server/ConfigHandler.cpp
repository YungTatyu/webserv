...
const config::Server& ConfigHandler::searchServerConfig(const struct TiedServer& tied_servers,
  ...
  for (size_t si = 0; si < tied_servers.servers_.size(); si++) {
    if (tied_servers.servers_[si]->server_name.getName().find(server_name) !=
...
const std::string getCurrentTimeLogFormat() {
  ...
  oss << days[gmTime->tm_wday] << "/" << months[gmTime->tm_mon] << "/" << 1900 + gmTime->tm_year << ":"
...
const std::string ConfigHandler::createAcsLogMsg(const uint32_t ip, const long status,
  ...
  ss << Utils::ipToStr(ip) << " - - [" << getCurrentTimeLogFormat() << "] \"" << requestMethod << " "
     << request.uri << " HTTP/1.1\" " << status << " \"" << requestUrl << "\" \"" << userAgent << "\""
     << std::endl;
