...
void SAME_OBJECT(const config::TK_TYPE type, const std::string value, const unsigned int line,
...
TEST(ParserDirectiveTest, AccessLog) {
  config::Token dir("access_log", config::TK_STR, 1), arg("logs/access.log", config::TK_STR, 1),
      semi("", config::TK_SEMICOLON, 1);
