...
TEST(clientMaxBodySizeTest, longMax) {
  ...
  EXPECT_EQ(http.client_max_body_size_.getSize().size_in_bytes_, kLongMax);
  test::test_directives_set(http.directives_set_, kClientMaxBodySize, true);
}
...
TEST(clientMaxBodySizeTest, longMaxKilo) {
  ...
  EXPECT_EQ(http.client_max_body_size_.getSize().size_in_bytes_, 9007199254740991 * config::Size::kilobytes);
  test::test_directives_set(http.directives_set_, kClientMaxBodySize, true);
}
...
TEST(clientMaxBodySizeTest, longMaxMega) {
  ...
  EXPECT_EQ(http.client_max_body_size_.getSize().size_in_bytes_, 8796093022207 * config::Size::megabytes);
  test::test_directives_set(http.directives_set_, kClientMaxBodySize, true);
}
...
TEST(clientMaxBodySizeTest, zero) {
  ...
  EXPECT_EQ(http.client_max_body_size_.getSize().size_in_bytes_, 0);
  test::test_directives_set(http.directives_set_, kClientMaxBodySize, true);
}
...
TEST(clientMaxBodySizeTest, notFound) {
  ...
  test::test_directives_set(http.directives_set_, kClientMaxBodySize, false);
}
