...
TEST(allowTest, ipv4)
{
	...
	// http
	test::test_value(http.access_list, {"all", "1.1.1.1/30"}, {config::ALLOW, config::ALLOW});
	test::test_directives_set(http.directives_set, kAllow, true);

	// server
	test::test_value(http.server_list[0].access_list, {"87.65.43.21/32", "87.65.0.21/2"}, {config::ALLOW, config::ALLOW});
	...
	test::test_value(http.server_list[0].location_list[1].access_list, {"192.0.2.1", "10.0.0.1"}, {config::ALLOW, config::ALLOW});
...
TEST(allowTest, ipv6)
{
	...
	// http
	test::test_value(http.access_list, {"2001:0db8::1", "fe80::2"}, {config::ALLOW, config::ALLOW});
	test::test_directives_set(http.directives_set, kAllow, true);

	// server
	test::test_value(http.server_list[0].access_list, {"::3", "::5", "all"}, {config::ALLOW, config::ALLOW, config::ALLOW});
	test::test_directives_set(http.server_list[0].directives_set, kAllow, true);
