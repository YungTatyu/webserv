...
{
	const config::Main	*config = config::init_config("test/conf/directives_parser/userid_expires/1.conf");
...
TEST(useridExpiresTest, notFound) {
	...
	test::test_directives_set(events.directives_set, kUseridExpires, false);
}
