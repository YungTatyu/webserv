...
TEST(UseTest, select)
{
	const config::Main	*config = config::init_config("test/conf/directives_parser/use/1.conf");
...
TEST(UseTest, poll)
{
	const config::Main	*config = config::init_config("test/conf/directives_parser/use/2.conf");
	ASSERT_NE(config, nullptr);
...
TEST(UseTest, kqueue)
{
	const config::Main	*config = config::init_config("test/conf/directives_parser/use/3.conf");
...
TEST(UseTest, epoll)
{
	const config::Main	*config = config::init_config("test/conf/directives_parser/use/4.conf");
