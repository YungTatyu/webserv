...
{
	const config::Main	*config = config::init_config("test/conf/directives_parser/userid_domain/1.conf");
...
TEST(useridDomainTest, notFound) {
	...
	test::test_directives_set(events.directives_set, kUseridDomain, false);
}
