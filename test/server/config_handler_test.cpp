...
TEST_F(ConfigHandlerTest, searchKeepaliveTimeout) {
  ...
  EXPECT_TRUE(test::sameTime(config_handler_.config_->http.keepalive_timeout.getTime(),
                             config_handler_.searchKeepaliveTimeout(tied_server_1, "first_server", "/")));
  // location set
  EXPECT_TRUE(test::sameTime(
      config_handler_.config_->http.server_list[0].location_list[1].keepalive_timeout.getTime(),
      config_handler_.searchKeepaliveTimeout(tied_server_1, "first_server", "/hello/")));
  ...
  EXPECT_TRUE(test::sameTime(config_handler_.config_->http.server_list[1].keepalive_timeout.getTime(),
                             config_handler_.searchKeepaliveTimeout(tied_server_2, "second_server", "/")));
}
...
TEST_F(ConfigHandlerTest, searchSendTimeout) {
  ...
  EXPECT_TRUE(test::sameTime(config_handler_.config_->http.send_timeout.getTime(),
                             config_handler_.searchSendTimeout(tied_server_1, "first_server", "/")));
  // location set
  EXPECT_TRUE(
      test::sameTime(config_handler_.config_->http.server_list[0].location_list[1].send_timeout.getTime(),
                     config_handler_.searchSendTimeout(tied_server_1, "first_server", "/hello/")));

  struct TiedServer tied_server_2("127.0.0.2", 8002);
  tied_server_2.servers_.push_back(&config_handler_.config_->http.server_list[1]);
  // server set
  EXPECT_TRUE(test::sameTime(config_handler_.config_->http.server_list[1].send_timeout.getTime(),
                             config_handler_.searchSendTimeout(tied_server_2, "second_server", "/")));
}
...
TEST_F(ConfigHandlerTest, searchUseridExpires) {
  ...
  EXPECT_TRUE(test::sameTime(config_handler_.config_->http.userid_expires.getTime(),
                             config_handler_.searchUseridExpires(tied_server_1, "first_server", "/")));
  // location set
  EXPECT_TRUE(
      test::sameTime(config_handler_.config_->http.server_list[0].location_list[1].userid_expires.getTime(),
                     config_handler_.searchUseridExpires(tied_server_1, "first_server", "/hello/")));

  struct TiedServer tied_server_2("127.0.0.2", 8002);
  tied_server_2.servers_.push_back(&config_handler_.config_->http.server_list[1]);
  // server set
  EXPECT_TRUE(test::sameTime(config_handler_.config_->http.server_list[1].userid_expires.getTime(),
                             config_handler_.searchUseridExpires(tied_server_2, "second_server", "/")));
}
...
TEST_F(ConfigHandlerTest, writeAcsLog) {
  ...
  msg = "aiueo";
  config_handler_.writeAccessLog(tied_server_1, "first_server", "/", msg);

  // ロケーションブロックで指定されたところへ出力
  msg = "kakikukeko";
  config_handler_.writeAccessLog(tied_server_1, "first_server", "/hello/", msg);

  // 親ブロックで指定されたファイルに出力
  msg = "sashisuseso";
  config_handler_.writeAccessLog(tied_server_1, "first_server", "/goodnight/", msg);

  struct TiedServer tied_server_2("127.0.0.2", 8002);
  tied_server_2.servers_.push_back(&config_handler_.config_->http.server_list[1]);
  // 親の親ブロックで指定されたファイルに出力
  msg = "tachitsuteto";
  config_handler_.writeAccessLog(tied_server_2, "second_server", "/", msg);
...
TEST_F(ConfigHandlerTest, writeErrLog) {
  ...
  file = "/dev/null";
  msg = "aiueo\n";
  config_handler_.writeErrorLog(tied_server_1, "first_server", "/", msg);

  // location.logに出力
  msg = "kakikukeko\n";
  config_handler_.writeErrorLog(tied_server_1, "first_server", "/hello/", msg);

  struct TiedServer tied_server_2("127.0.0.2", 8002);
  tied_server_2.servers_.push_back(&config_handler_.config_->http.server_list[1]);
  // default fileに出力
  msg = "sashisuseso\n";
  config_handler_.writeErrorLog(tied_server_2, "second_server", "/", msg);
