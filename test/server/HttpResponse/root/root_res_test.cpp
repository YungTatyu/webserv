...
TEST(HttpResponseRoot, path_info4) {
  ...
  ASSERT_NO_FATAL_FAILURE(test.setUpAll({{"127.0.0.1", 4240}, {"127.0.0.1", 4241}},
                                        {{"host", "test"}, {"User-Agent", "Mozilla/5.0"}},
                                        {config::REQUEST_METHOD::GET, config::REQUEST_METHOD::POST}, "/cgi-bin/path_info.php", HttpRequest::PARSE_COMPLETE));

  // test member variables
  ASSERT_EQ(test.responses_[0].state_, HttpResponse::RES_EXECUTE_CGI);
  ASSERT_EQ(test.responses_[0].res_file_path_, "/cgi-bin/path_info.php");
  ASSERT_EQ(test.responses_[0].path_info_, "");
}
...
TEST(HttpResponseRoot, path_info5) {
  ...
  ASSERT_NO_FATAL_FAILURE(test.setUpAll({{"127.0.0.1", 4240}, {"127.0.0.1", 4241}},
                                        {{"host", "test"}, {"User-Agent", "Mozilla/5.0"}},
                                        {config::REQUEST_METHOD::GET, config::REQUEST_METHOD::POST}, "/cgi-bin/path_info.php/path1/path2/", HttpRequest::PARSE_COMPLETE));

  // test member variables
  ASSERT_EQ(test.responses_[0].state_, HttpResponse::RES_EXECUTE_CGI);
  ASSERT_EQ(test.responses_[0].res_file_path_, "/cgi-bin/path_info.php");
  ASSERT_EQ(test.responses_[0].path_info_, "/path1/path2/");
}
