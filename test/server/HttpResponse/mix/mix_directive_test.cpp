...
class HttpResponseMix : public ::testing::Test {
 protected:
  ...
  void initRequest(const test::string_map_case_insensitive &headers,
                   const config::REQUEST_METHOD &method, const std::string &uri,
...
TEST_F(HttpResponseMix, try_files_and_alias) {
  initRequest({{"host", "test_server"}, {"User-Agent", "Mozilla/5.0"}},
               config::REQUEST_METHOD::GET, "/alias/",
...
TEST_F(HttpResponseMix, try_files_and_root_in_loc) {
  initRequest({{"host", "test_server"}, {"User-Agent", "Mozilla/5.0"}},
               config::REQUEST_METHOD::GET, "/location-root/",
               HttpRequest::PARSE_COMPLETE);
  tied_server_ = config_handler_.createTiedServer("127.0.0.1", 4242);
...
TEST_F(HttpResponseMix, try_files_internal_redirect) {
  initRequest({{"host", "test_server"}, {"User-Agent", "Mozilla/5.0"}},
               config::REQUEST_METHOD::GET, "/redirect/",
               HttpRequest::PARSE_COMPLETE);
  tied_server_ = config_handler_.createTiedServer("127.0.0.1", 4242);
...
TEST_F(HttpResponseMix, try_files_error_page) {
  initRequest({{"host", "test_server"}, {"User-Agent", "Mozilla/5.0"}},
               config::REQUEST_METHOD::GET, "/code-error-page/",
               HttpRequest::PARSE_COMPLETE);
  tied_server_ = config_handler_.createTiedServer("127.0.0.1", 4242);
