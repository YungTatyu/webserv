...
TEST(HttpResponseTry_files, internal_redirect_dir_nothing) {
  ...
  ASSERT_NO_FATAL_FAILURE(test.setUpAll(
      ...
      {config::REQUEST_METHOD::GET, config::REQUEST_METHOD::HEAD}, "/no-dir/", HttpRequest::PARSE_COMPLETE));
...
TEST(HttpResponseTry_files, internal_redirect_file_nothing) {
  ...
  ASSERT_NO_FATAL_FAILURE(test.setUpAll(
      {{"127.0.0.1", 4244}, {"127.0.0.1", 4245}}, {{"host", "mstk"}, {"User-Agent", "Mozilla/5.0"}},
      {config::REQUEST_METHOD::GET, config::REQUEST_METHOD::HEAD}, "/no-file/", HttpRequest::PARSE_COMPLETE));
...
TEST(HttpResponseTry_files, error_code) {
  ...
  ASSERT_NO_FATAL_FAILURE(test.setUpAll(
      ...
      {config::REQUEST_METHOD::GET, config::REQUEST_METHOD::HEAD, config::REQUEST_METHOD::POST}, "/", HttpRequest::PARSE_COMPLETE));
  const std::string expect_body = test.createDefaultErrorBody(404);
  test.testHeaders({
      {"Server", "webserv/1.0"},
      {"Date", ""},
