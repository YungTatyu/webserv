...
void assert_allowRequest_loop(
      ConfigHandlerTest& test,
      int server_key,
      int loc_key,
      std::vector<config::REQUEST_METHOD> vec_method,
      ...
      std::vector<std::vector<int>> vec_vec_expect)
{
  ...
  const config::Location* location = &test.config_handler_.config_->http.server_list[server_key].location_list[loc_key];
  
  method_it method = vec_method.begin();
  vec_expect_it vec_expect = vec_vec_expect.begin();
  for (;
      method != vec_method.end() && vec_expect != vec_vec_expect.end();
      method++, vec_expect++)
  {
    ...
    for (;
        ip != vec_ip.end() && expect != (*vec_expect).end();
        ip++, expect++)
    {
      EXPECT_EQ(*expect, test.config_handler_.allowRequest(server, location, test.request_, test.createClient(*ip)));
    }
  }
}
}; // namespace test
...
TEST(ConfigHandlerTest, allowRequest_limit_except_POST)
{
  ...
  test::assert_allowRequest_loop(
          test,
          0,
          0,
          {config::GET, config::POST, config::DELETE, config::HEAD},
