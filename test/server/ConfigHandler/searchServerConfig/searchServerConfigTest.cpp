...
void  sameServer(const config::Server& expect, const config::Server& actual) {
  ...
  for (;
        expect_lit != expect.listen_list.end(), actual_lit != actual.listen_list.end();
  ...
  for (;
        expect_nit != expect.server_name.getName().end(), actual_nit != actual.server_name.getName().end();
