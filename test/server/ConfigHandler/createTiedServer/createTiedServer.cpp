...
TEST(ConfigHandlerTestCreateTiedServer, three_server) {
  ...
  EXPECT_TRUE(test::sameTiedServer(
              test.tied_server_,
              test.config_handler_.createTiedServer("127.0.0.1", 8001)
              ));
}
...
TEST(ConfigHandlerTestCreateTiedServer, one_server1) {
  ...
  EXPECT_TRUE(test::sameTiedServer(
              test.tied_server_,
              test.config_handler_.createTiedServer("127.0.0.2", 8002)
              ));
}
...
TEST(ConfigHandlerTestCreateTiedServer, one_server2) {
  ...
  EXPECT_TRUE(test::sameTiedServer(
              test.tied_server_,
              test.config_handler_.createTiedServer("127.0.0.3", 8003)
              ));
}
