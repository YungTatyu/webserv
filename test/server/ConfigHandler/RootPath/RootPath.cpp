...
TEST(ConfigHandlerTestSearchRootPath, root) {
  ...
  const config::Server& server = test.config_handler_.config_->http_.server_list_[0];
  const config::Location* location = &test.config_handler_.config_->http_.server_list_[0].location_list_[0];
...
TEST(ConfigHandlerTestSearchRootPath, alias) {
  ...
  const config::Server& server = test.config_handler_.config_->http_.server_list_[0];
  const config::Location* location = &test.config_handler_.config_->http_.server_list_[0].location_list_[1];
...
TEST(ConfigHandlerTestSearchRootPath, server) {
  ...
  const config::Server& server = test.config_handler_.config_->http_.server_list_[0];
  const config::Location* location = &test.config_handler_.config_->http_.server_list_[0].location_list_[2];
...
TEST(ConfigHandlerTestSearchRootPath, http) {
  ...
  const config::Server& server = test.config_handler_.config_->http_.server_list_[1];
...
TEST(ConfigHandlerTestSearchRootPath, relative_path) {
  ...
  const config::Server& server = test.config_handler_.config_->http_.server_list_[0];
  const config::Location* location = &test.config_handler_.config_->http_.server_list_[0].location_list_[3];
